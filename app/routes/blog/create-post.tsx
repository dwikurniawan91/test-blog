          ...
          <div className="mt-4">
            ...
            <div className="sm:hidden text-center">
                ...
                <div className="mt-2 flex items-center space-x-1">
                    {steps.map(step => (
                        <div key={step.number} className={`flex-1 h-2 rounded-full ${currentStep >= step.number ? 'bg-gray-900 dark:bg-gray-50' : 'bg-gray-200 dark:bg-gray-700'}`}></div>
                    ))}
                </div>
            </div>
            ...
            <div className="hidden sm:flex sm:space-x-2">
                {steps.map(step => (
                    <div key={step.number} className="flex-1 text-center text-sm">
                        <div className={`py-2 border-b-4 transition-colors duration-300 ${currentStep >= step.number ? 'border-gray-900 dark:border-gray-50' : 'border-gray-200 dark:border-gray-700'}`}>
                            <span className={`font-semibold transition-colors duration-300 ${currentStep >= step.number ? 'text-gray-900 dark:text-gray-50' : 'text-gray-400'}`}>Step {step.number}</span>
